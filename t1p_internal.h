...
typedef struct optpr_problem_t {
    ...
    uint_t sizeJ; /* size of litab % d�termine la complexit� du probl�me */
...
void optpr_build(t1p_internal_t* pr, itv_t alphaix, itv_t alphaiy, t1p_nsym_t* pnsym, itv_t nsymItv1, itv_t nsymItv2, indicesSets_t I); /* construire le probl�me � r�soudre */
...
*/
{
	...
	if (exp1->q || exp2->q) {
		...
		if (pnsym) {
			...
			/* J */
			//itv_sub(tmp, alphaix, alphaiy); /* alpha_i^x - alpha_i^y */
			optpr_build(pr, alphaix, alphaiy, pnsym, nsymItv1, nsymItv2, J); /* construire le probl�me � r�soudre */
		    }
		    pnsym = NULL;
		}
	    }
...
static inline t1p_aff_t * t1p_aff_join_constrained1(t1p_internal_t* pr, t1p_aff_t *exp_a, t1p_aff_t *exp_b, t1p_t* a, t1p_t* b, t1p_t* ab)
{
	...
	if (exp1->q && exp2->q) {
		...
		fatal("a�e, a�e, a�e, le beta est n�gatif !\n");
	    }
